...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-golden-light">
      <header className="px-4 py-4 sm:px-6 md:px-12 md:py-6 flex items-center justify-between bg-golden-dark">
        <div className="flex items-center gap-3 sm:gap-4">
          {showBackButton && (
            <Link 
              to={getBackPath()}
              className="text-golden-cream hover:text-white transition-colors duration-300"
              aria-label="Voltar"
            >
      ...
      <footer className="mt-auto px-4 py-6 sm:px-6 sm:py-8 md:px-12 text-center text-sm text-brown-dark/80">
